...
const Activities = ({ scholarships, awards, childSelector, selectedChild }) => {
  return (
    <div className="space-y-6">
      <h2 className="text-2xl font-bold text-[#0F172A]">Scholarships & Awards</h2>
      {childSelector()}

      {selectedChild && (
        <p className="text-sm text-[#64748B]">
          Showing achievements for <span className="font-semibold text-[#0F172A]">{selectedChild.userId?.name || 'Student'}</span>
        </p>
      )}
      ...
      <div className="bg-white rounded-xl border border-slate-200 overflow-hidden">
        <div className="px-6 py-4 border-b border-slate-100 flex items-center gap-2">
          ...
          <h3 className="font-semibold text-[#0F172A]">Scholarships</h3>
        </div>
        {scholarships.length === 0 ? (
          <div className="p-12 text-center text-[#64748B]">
            <Trophy className="mx-auto mb-4 text-gray-300" size={48} />
            <p className="text-lg font-medium">No scholarships yet</p>
            <p className="text-sm mt-1">Scholarships awarded to your child will appear here</p>
          </div>
        ) : (
      ...
      <div className="bg-white rounded-xl border border-slate-200 overflow-hidden">
        <div className="px-6 py-4 border-b border-slate-100 flex items-center gap-2">
          ...
          <h3 className="font-semibold text-[#0F172A]">Awards & Prizes</h3>
        </div>
        {awards.length === 0 ? (
          <div className="p-12 text-center text-[#64748B]">

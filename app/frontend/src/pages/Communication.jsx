...
const Communication = () => {
  ...
  useEffect(() => {
    api.get('/api/classes').then(res => setClasses(res.data?.data || res.data || [])).catch(() => {});
  }, []);

  useEffect(() => {
    if (activeTab === 'ptm') {
      api.get('/api/ptm').then(res => setPtmList(res.data?.data || res.data || [])).catch(() => {});
    }
  }, [activeTab]);
  ...
  const handlePtmSubmit = async (e) => {
    ...
    setPtmLoading(true);
    try {
      const res = await api.post('/api/ptm', ptmForm);
      toast.success('PTM scheduled successfully!');
      setPtmList(prev => [...prev, res.data?.data || res.data]);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex gap-2 border-b-2 border-[#FCD34D] overflow-x-auto">
        {['email', 'sms', 'whatsapp', 'announcements', 'circulars', 'messages', 'ptm'].map(tab => (
          <button
            key={tab}
